    ...
    address[] addresses;
     ...
     function getAddresses() constant returns (address[]) {
      ...
      function createTweetWallet(string _username) public {
          address newTweetWallet = new TweetWallet(_username);
          addresses.push(newTweetWallet);
      }
}
...
contract TweetWallet is usingOraclize {
    ...
    function __callback(bytes32 myid, string result) {
        ...
        // Remove first and last parts of string returned by oraclize
        var redemptionTweetAsSlice = redemptionTweet.toSlice();
        redemptionTweetAsSlice.beyond("[\"\\n  \", \"".toSlice()).until("\", \"\\n\"]".toSlice()); // "B C B"

        // Split syntax in two addresses.
        var claimerAddress = redemptionTweetAsSlice;
        ...
        newTweetInfo(claimerAddressString);

        // Built-in Oraclize function
    ...
    function claim(string _statusId) {
        // html(https://twitter.com/MultisigIt/status/948276796251934720).xpath(//*[contains(@class, 'tweet-text')]/text())
        // String will be prefix + username + backslash + statusId + suffix
        string memory prefix = "html(https://twitter.com/";
        string memory forwardSlash = "/status/";
        string memory suffix = ").xpath(//*[contains(@class, 'tweet-text')]/text())";


        bytes memory _ba = bytes(prefix);
        bytes memory _bb = bytes(username);
        bytes memory _bc = bytes(forwardSlash);
        bytes memory _bd = bytes(_statusId);
        bytes memory _be = bytes(suffix);
        ...
        string memory url = string(babcde);
